...
public class ListUtils {
	...
	public static <T> List<T> mergeAllList(List<T>... list) {
		for (List<T> ts : list) {
			LogPrintUtils.info(log, "ts.size() = " + ts.size());
		}
		int i = 0;
	...
	/**
	 * 给 sourceList 复制属性，将对象转换成 targetClass 对象
	 *
	 * @param sourceList
	 * @param targetClass
	 * @param <T>
	 * @return
	 */
	public static <T, H> List<H> listCopyProperties(List<T> sourceList, Class<H> targetClass) {
		List<H> resultList = new ArrayList<>();
		sourceList = filterEmpty(sourceList);
		try {
			for (T t : sourceList) {
				H newInstance = targetClass.newInstance();
